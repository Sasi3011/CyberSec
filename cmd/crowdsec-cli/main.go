...
func (cli *cliRoot) NewCommand() (*cobra.Command, error) {
	...
	cmd := &cobra.Command{
		...
		Short: branding.CLIName + " allows you to manage " + branding.PlatformName,
		Long: branding.CLIName + " is the main command to interact with your " + branding.PlatformName + " service, scenarios & db.\n" +
			"It is meant to allow you to manage bans, parsers/scenarios/etc, api and generally manage your " + branding.PlatformName + " setup.",
		ValidArgs:         validArgs,
	...
	pflags.StringVarP(&ConfigFilePath, "config", "c", csconfig.DefaultConfigPath("config.yaml"), "path to "+branding.PlatformName+" config file")
