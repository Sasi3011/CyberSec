...
func leavingMessage(w io.Writer) {
	fmt.Fprintln(w, "Leaving CyberSec configuration.")
...
func (cli *cliSetup) wizard(ctx context.Context, detectConfig *setup.DetectConfig, opts setup.DetectOptions, acquisDir string, interactive bool, dryRun bool, logger logrus.FieldLogger) error {
	...
	if !dryRun && len(userAcquisFiles) != 0 {
		fmt.Fprintln(os.Stdout, "The following acquisition files were not generated by CyberSec, or were altered:")
...
CyberSec alone will not block any IP address. If you want to block them, you must use a remediation component.
You can find them on`), yellowBold("CyberSec Cloud Console"))
