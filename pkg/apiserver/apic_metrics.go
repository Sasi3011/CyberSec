...
func (a *apic) SendMetrics(ctx context.Context, stop chan bool) {
	...
	log.Infof("Start sending metrics to %s (interval: %s once, then %s)",
		branding.CentralAPI, metInts[1].Round(time.Second), metInts[2])
...
func (a *apic) SendUsageMetrics(ctx context.Context) {
	...
	log.Debugf("Start sending usage metrics to %s (interval: %s once, then %s)", branding.CentralAPI, a.usageMetricsIntervalFirst, a.usageMetricsInterval)
