...
func (a *apic) Push(ctx context.Context) error {
	...
	log.Infof("Start push to %s (interval: %s once, then %s)", branding.CentralAPI, a.pushIntervalFirst.Round(time.Second), a.pushInterval)
...
func (a *apic) Pull(ctx context.Context) error {
	...
	log.Infof("Start pull from %s (interval: %s once, then %s)", branding.CentralAPI, a.pullIntervalFirst.Round(time.Second), a.pullInterval)
